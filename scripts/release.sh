...
KNOWLEDGE_CARGO_TOML="knowledge/Cargo.toml"
KNOWLEDGE_CARGO_TOML_LOCK="knowledge/Cargo.lock"

CURRENT_VERSION=$(grep '^VERSION=' "$MCP_SCRIPT" | sed 's/VERSION="//;s/"//')

echo "Current version: ${CURRENT_VERSION}"
read -rp "New version: " VERSION

sed -i '' "s/^VERSION=\".*\"/VERSION=\"${VERSION}\"/" "$MCP_SCRIPT"
sed -i '' "s/^version = \".*\"/version = \"${VERSION}\"/" "$KNOWLEDGE_CARGO_TOML"
sed -i '' "s/\"version\": \".*\"/\"version\": \"${VERSION}\"/" "$CLAUDE_PLUGIN_JSON"
sed -i '' "s/\"version\": \".*\"/\"version\": \"${VERSION}\"/" "$CLAUDE_MARKETPLACE_JSON"
sed -i '' "s/^VERSION=\".*\"/VERSION=\"${VERSION}\"/" "$DASHBOARD_SCRIPT"
