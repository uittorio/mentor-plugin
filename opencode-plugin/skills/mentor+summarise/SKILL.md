...
Look in the current conversation history for a `session_id` stored by the `mentor+` skill, and the accumulated session markdown maintained in context. Use the in-context markdown as the source material.
